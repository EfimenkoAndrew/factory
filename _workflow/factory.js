...
async function runItem(item) {
  ...
  // KI-E144/KI-E144B fix (ported from a host-mount session). KI-E144: a bare `/⚠\s*MAIN-DRIFT/`
  // regex matched ANY substring, but `driver.mjs main-check <id>` ALWAYS appends a SECOND, GLOBAL,
  // unconditional sweep after the per-id result — `⚠ MAIN-DRIFT unclaimed (KI-E89): ...` — which
  // fires on ANY stray path ANYWHERE in the repo; one genuine stray file from ONE item's agent made
  // the SAME "unclaimed" line appear in EVERY other concurrently-running item's own note too (live
  // there: a 6-item batch, five unrelated services, all failed off one incident). Anchoring the regex
  // to the item's own id fixed that, but KI-E144B found the anchored regex STILL prose-parsing: it
  // has no concept of negation, so a runner correctly determining its own check is clean and quoting
  // the marker format in its note to PROVE its absence ("no such line printed") gets misread as a hit
  // by the same anchored match. Durable fix: stop parsing prose for a verdict at all — read the
  // runner's own closed `mainDriftOwnFiles` boolean and nothing else.
