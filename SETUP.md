...
| **Host policies** | `policies` in `config/factory.config.json` — `noNewComments` (KI-E57/KI-E59 zero-new-comments rule + its mechanical gate), `noSchemaChanges` (KI-E58 no-migrations hard stop), and `archaeology` (KI-E75 — a research-phase role that establishes validated ground truth for a doc-less target before planning, then writes it into the doc conventions below), ALL OFF in the shipped engine | flip per host in the gitignored local overlay: `{ "policies": { "noNewComments": true, "noSchemaChanges": true, "archaeology": true } }` — the driver prints the effective state at every `group`/`sweep`; `archaeology` on is the right default for a large, undocumented legacy host, off for one that already documents itself |
...
| **Doc conventions** | prompt enrichment looks for `doc/data-flows/<target>.md`, `<target>/CONTEXT.md`, `<target>/AGENTS.md`; graph-audit lints a `STANDARDS-LEDGER.md` path | all best-effort — absent files just yield no enrichment; with `policies.archaeology` on, an EMPTY set of these for a target is also the trigger that runs the archaeologist role (KI-E75), which then writes validated findings into whichever of them fits (or the host's own visibly different convention, if it has one) |
