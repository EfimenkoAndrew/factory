# Real-infra verification — Testcontainers harness notes

> Status: **ACTIVE.** The binding gate is wired in `factory.js`: a `realInfra=true`
...
Typical real examples: a read-then-append TOCTOU double-spend on a balance reserve, a
consumer-agnostic idempotency-key collision silently dropping a projection, an
unqualified-schema SQL error (`42P01`) swallowed by a background reaper. Every one
passes in-memory.
...
proof. Docker-absent environments: the item parks `BLOCKED:needs-docker` (never a silent
pass), surfaced in `queue/decisions.md`.

## Driver hook

`factory.config.json:realInfraThemes` + the work item's `realInfra` flag select this path. The
runner template (`agents/runner.md`) already asks for `realInfraExercised`; the gate makes it
